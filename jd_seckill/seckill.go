...
func (this *Seckill) MakeReserve() {
	...
	resp, body, err := req.SetUrl("https://yushou.jd.com/toYuyue.action?sku=" + skuId + "&key=ac8d39a2c186adcbe32ba8ac1d5233f5&shopId=652099&isPlusLimit=0").SetMethod("get").Send().End()
	log.Println(body)
	if err != nil || resp.StatusCode != http.StatusOK {
		fmt.Println(err)
		log.Println("预约商品失败")
