...
func getJdTime() (int64, error) {
	...
	resp, body, err := req.SetUrl("https://www.jd.com").SetMethod("get").Send().End()
	if err != nil || resp.StatusCode != http.StatusOK {
		log.Println("获取京东服务器时间失败")
		return 0, errors.New("获取京东服务器时间失败")
	}
	// 之前的获取方法已失效，以下是最新方法
	var imgRE = regexp.MustCompile("window.pageConfig.serverTime = new Date\\('(.+)\\'\\)")
	imgs := imgRE.FindAllStringSubmatch(body, -1)
	out := make([]string, len(imgs))
	var jdTimeString string
	for i := range out {
		out[i] = imgs[i][1]
		jdTimeString = out[i]
	}
	loc, _ := time.LoadLocation("Asia/Shanghai")
	tt, _ := time.ParseInLocation("2006-01-02 15:04:05", jdTimeString, loc)
	return tt.Unix(), nil
}
